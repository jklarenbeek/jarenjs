...
| [`series.js`](./series.js) | The temporal ground: one range, one fixed bucketing, one rolling window and one as-of read over a seeded series, answered by plain references, by a generic query document and by stock SQLite under a declared epoch column — gated on the committed series corpus and on the routes agreeing with each other, with the resident ceiling and the durable loss both published, and no kernel row, because there is no kernel yet | Deciding what a temporal fast path has to beat, and what a declared epoch column buys over the document it came from |
