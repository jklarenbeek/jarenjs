...
export const HOME_CONTENT = {
  ...
  engines: [
    ...
    { key: 'query', title: 'JSON Query', blurb: 'XQuery 3.1 semantics — FLWOR, joins, grouping, quantifiers, folds and windows — as JSON documents with JSONPath leaves. Uncorrelated equijoins are planned as hash joins, and only where the rewrite is provably invisible.', perf: '17–259× vs fontoxpath, 7–53× vs JSONata' },
    { key: 'jslt', title: 'JSLT', blurb: 'The stylesheet layer: JSONPath matches position, JSON Schema matches shape, query documents produce output. Identity transforms return the input reference. Operator packs register more — createJsltRegistry().use(mathPack)… adds $npv, $mean, $sqrt to JSLT, query and linq, and over @jarenjs/db the scalar subset pushes to SQLite as UDFs.', perf: 'proof-of-no-change in 24–81 ns; host operator packs are opt-in' },
