...
- **Docs & Examples.** Thirty-two documentation sections as a content document (`src/content/docs.js`, kind-nodes — an LLM constrained to the schemas could write these pages), and the curated example library lives in `@jarenjs/play` (the rail on `#/play`).
