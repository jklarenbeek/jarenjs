...
- **The experiment IDE.** The Studio saves named documents (`localStorage` behind the injected `storage` env); play sessions save through their own store. Legacy experiments from the retired `#/playground` still load — they translate into the equivalent play session. The store is plain JSON — exportable, syncable later.
