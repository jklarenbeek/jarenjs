...
| `src/path.js` | the JSONPath compiler: parser, nodes-mode compilers (normalized paths), public API, the pointer bridge |
...
- **The `0#` hash form yields the member *name*.** The `#` form resolves to the location's last segment **as a string** (`''` at the root), a deliberate divergence from draft-luff-relative-json-pointer, which yields a *number* for an array position. The string form is kept because the validator's `$data`/`data` keyword tests rely on it; `compileRelativeJSONPointer`'s JSDoc records the contract.
