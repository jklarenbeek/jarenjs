...
## Formula and review composition

The formula envelope compiles through the existing Query normalization/closure engine. Core owns snapshot/cache primitives; formulas add versioned schema/helper identities and bounded row outcomes. Migration only recognizes a measured return subset and preserves original records. Rules compose the batch evaluator and pointer readers into immutable plans; injected application commands own all writes and receipts. No component or validator is imported into JSON.
