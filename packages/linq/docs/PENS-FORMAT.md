...
`/flow`, `/app` and `/forms`); `.` stays the chain. This document
...
| Document (grammar) | Home | Pen |
|---|---|---|
| JSON Schema (+ `$query`, `errorMessage`/`$msgid`, `x-coerce`/`x-trim`) | `@jarenjs/validate` | **`./schema`** (§2) |
| `x-entity` on JSON Schema; `$model` 0.1 (`jaren-model`) | `@jarenjs/db` | **`./model`** (§3) |
| query documents (`jaren-query`) | `@jarenjs/json` | **the chain**, `.` — [LINQ-FORMAT](LINQ-FORMAT.md) |
| `$jslt` 0.1 (`jaren-jslt`) | `@jarenjs/json` | **`./jslt`** (§4) |
| `$migration` 0.1 (`jaren-migration`) | `@jarenjs/db` | **`./migration`** (§5) |
| `$contract` 0.1 (`jaren-contract`) | `@jarenjs/contract` | **`./contract`** (§7) |
| `$fsm` 0.1, `$dag` 0.1 (`jaren-fsm`, `jaren-dag`) | `@jarenjs/flow` | **`./flow`** (§8) |
| `jaren-app` 0.1 | `@jarenjs/app` | **`./app`** (§9) |
| `x-form` on JSON Schema | `@jarenjs/forms` | **`./forms`** (§10) |
...
| LIVE options, JOBS payloads | option objects AROUND a query or dag document; `live` is a client terminal (§6) |
