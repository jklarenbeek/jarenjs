...
`@jarenjs/linq/jslt` builds to **<!--fact:bundle.jslt-->19,798<!--/fact--> bytes** as a minified,
...
Two figures worth reading beside it: `./migration` (<!--fact:bundle.migration-->24,273<!--/fact--> bytes)
carries this pen's `body()` and pays for it, which is why the two prices
sit so close; and `./flow` (<!--fact:bundle.flow-->19,852<!--/fact--> bytes) is within 60 bytes of this one
