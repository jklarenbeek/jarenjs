 ...
 * @returns {string} The French display name, article included
 */
...
export const fr = {
  ...
  'form/const': (p) => `Doit être ${formatValue(p.constValue)}`,
  'form/enum': (p) => `Doit être ${Array.isArray(p.enumValues) ? listFormat.format(p.enumValues.map(formatValue)) : formatValue(p.enumValues)}`,
