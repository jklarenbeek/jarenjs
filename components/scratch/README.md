...
**A JSON-engine scratchpad — understand an engine before you compose it.**
...
building an application out of many files, `@jarenjs/scratch` is for
learning one engine standalone — a reference bench you experiment on first.

Two layers, the suite's convention:

- **the engine** (`@jarenjs/scratch`) — headless: an engine registry, a
  ...
  nothing of the DOM.
- **the component** (`@jarenjs/scratch/component`) — the scratchpad UI, a
  `createScratchComponent()` factory (like `@jarenjs/calc`): a JSLT view
...
See [docs/SCRATCH-FORMAT.md](docs/SCRATCH-FORMAT.md).

## Install

```
npm install @jarenjs/scratch
```
