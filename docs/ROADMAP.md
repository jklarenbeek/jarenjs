...
The agreement corpus proves the model agrees with the validator for the cases
it covers. These are the constructs where it does not yet, found by an external
review reading the emitter against `compileNormalizer` and the specification.
Each one makes a generated type disagree with runtime, which is the only defect
class this package really has.

- [ ] **Normalization analysis must match `compileNormalizer` exactly** — the
  emitter descends `anyOf`/`oneOf` when deriving the normalized variant; the
  runtime normalizer deliberately does not. A generated normalized type can
  therefore require output the runtime never produces.
- [ ] **Required properties with defaults** — a required property with an
  enabled default may be omitted at runtime and materialized by normalization,
  but the accepted type still demands it.
- [ ] **`enum`/`const` versus accepted-side coercion** — literal types are
  chosen before coercion widening, so runtime accepts and normalizes `"1"` to an
  integer enum member while the accepted type rejects it.
- [ ] **Tuple cardinality and rest semantics** — every `prefixItems` entry is
  emitted required, and an omitted `items`/`additionalItems` is treated as
  closed although JSON Schema accepts shorter and longer arrays. An empty
  prefix with a rest schema can emit invalid TypeScript (`[, ...Array<T>]`).
- [ ] **Root and sibling `$ref`s** — a root `$ref` emits `unknown` for the root
  alias, `$ref` siblings are ignored in type construction, and plain-anchor
  refs are unsupported.
- [ ] **Stop inferring type from applicator keywords** — `properties` or
  `items` makes the emitter assume object or array. JSON Schema implies no such
  thing, so the validator accepts primitives the generated type rejects.
- [ ] **Closed empty object** — `type: 'object'` with `additionalProperties:
  false` and no members emits an empty interface, which TypeScript lets a
  primitive satisfy.
- [ ] **Normalized boolean roots** — normalization can produce duplicate
  declarations and a model root naming no emitted declaration; boolean `$defs`
  have related memoization problems.
- [ ] **Bundle-wide name resolution** — bundle mode concatenates declarations
  with no global name table, so repeated `$defs.Id` produce duplicate
  identifiers.
- [ ] **Fully typed public API** — generated declarations for
  `compileEmitModel` omit `normalize` and `variantSuffix`, `emitTypeScript`
  accepts a bare `object`, and the model/options types are not exported, so the
  programmatic selective route is unusable from TypeScript without an
  excess-property error.
- [ ] **Record the constraints that are dropped silently** — the recorded list
  omits integer-ness, `not`, conditionals, dependent/unevaluated keywords and
  pattern-key restrictions, so some widening is undocumented.
