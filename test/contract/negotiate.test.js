...
describe('negotiate — the compatibility matrix', () => {
  it('same-version: equal versions, and two unversioned contracts', async () => {
    const { client, urls } = pair(contractOf({ id: 'shop', version: '5' }), contractOf({ id: 'shop', version: '5', compat: ['4'] }));
    const n = await client.negotiate();
    assert.deepStrictEqual(n, { compatible: true, reason: 'same-version', server: { id: 'shop', version: '5', compat: ['4'], revision: null }, error: null });
    assert.deepStrictEqual(urls, [WELL_KNOWN_PATH]);
    const bare = await pair(contractOf({}), contractOf({})).client.negotiate();
    assert.deepStrictEqual([bare.compatible, bare.reason, bare.server], [true, 'same-version', { id: null, version: null, compat: [], revision: null }]);
  });
  ...
  it('version-mismatch: none of the rules hold — JC2057 names both versions', async () => {
    const n = await pair(contractOf({ id: 'shop', version: '6', compat: ['4'] }), contractOf({ id: 'shop', version: '5', compat: ['3'] })).client.negotiate();
    ...
    assert.deepStrictEqual(n.server, { id: 'shop', version: '5', compat: ['3'], revision: null });
