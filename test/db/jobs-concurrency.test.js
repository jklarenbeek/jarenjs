...
describe('exactly-once execution under contention', () => {
  it('4 workers on 4 connections execute 200 jobs exactly once (×3 rounds)', async (t) => {
    for (let round = 0; round < 3; round++) {
      ...
      // The test runner keeps the body failure even when its cleanup hook fails.
      // Always stop every worker and close its connection before removing files.
      t.after(release);
      producer = await openStore(MODEL,
