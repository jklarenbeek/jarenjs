...
- [x] **O(change) rendering** — **shipped, three layers**. (1) The changed-path feed: `json/patch`'s `changes: true` reports invalidation-sound pointers, surfaced to `createApp` subscribers. (2) **Memoized rule outputs**: the JSLT `memo` compile option caches rule bodies by (location, value reference) behind a compile-time eligibility analysis (no `$root`/`$path`/externals, transitively through `$apply`; no root refs in match filters) with a two-generation cache — an unchanged document returns the ENTIRE previous output by reference, and `@jarenjs/app` + the website's `memo1` viewModel derivations turn that into reference-equal vnodes the patcher skips in O(1). (3) **The `memo` marker** (VIEW-FORMAT §5.5): reference equality needs the *same object* on both sides, which a producer that rebuilds its tree cannot always give; an equal `memo` prop on two same-node vnodes is the declared alternative — the renderer skips the subtree without a prop diff or descent, sharing §5.1's soundness condition (suspended while any widget is poisoned, so a memo-stable subtree can never leave one inert). It is a producer-owned assertion, `key`'s sibling: a violated promise is stale output, the same class of error as a duplicate key. Measured (`node benchmark/view.js`): a reallocated parent over 10 000 shared children costs ~448 µs to scan for `===` skips and ~41 µs with the marker — 10.9×, and the gap widens with the child count.
- [ ] **Prepass-level pruning in the JSLT matcher** — the remaining O(change) idea, now standing on its own: skip re-evaluating match *paths* over unchanged regions using the changed-path feed directly, rather than discovering the reuse afterwards through the memo cache.
...
- [x] **JSON-Patch-based O(1) incremental re-render** — **shipped**:
  `createChartSession` (line + candlestick) consumes the adapter's RFC 6902
  change feed (`createStreamAdapter({ changes: true })` → `takeChanges()`,
  replay-equivalence tested) and patches only the touched series/candle
  groups, keeping every other child reference-equal for the patcher's
  O(1) skip. What unblocked it was not app support but a **domain-stability
  policy** (`config.domain`: quantized sliding window, pinned or
  step-quantized bounds): unit-space ASTs store positions as fractions of
  the domain, so a tick that moves the scales legitimately changes every
  mark — the wholesale rebuild on such a frame is the correct rendering,
  and `mode` reports which path ran. Byte equality with the wholesale
  render is asserted after every tick over thousands of property frames.
  Measured (`npm run benchmark:charts`, one appended point): ~8 µs at
  100×5, ~4.6 µs at 1 000×5, ~4.2 µs at 10 000×5 — flat in n, against a
  wholesale tick growing 319 µs → 1.03 ms → 11.3 ms. The Binance kline
  demo runs on it.
- [ ] **Sessions for the remaining streaming shapes** — `bar` (live
  counts/sums) is the next accumulator with an obvious incremental path;
  the other nine types re-render wholesale, which is correct and, at their
  sizes, cheap.
