...
- [~] **Rebuild the website on view + app** — in progress as `@jarenjs/webnext`, living beside `@jarenjs/website` until parity, then it takes the name and the deploy. Done: shell/routing/theme (pages as JSLT modes, tabs as links), benchmarks through the kind-node vocabulary (overview/validate-summary/pointer/patch/toml), the flagship JSON Schema playground (live compile, standard-forms-stylesheet form, EN/NL report-time errors), headless site tests, 9 kB gzip site chunk. Open: docs & examples content documents, the other eight playground engines, validate/jsonpath deep-dive tables + histograms, scenario matrices with program sources, share links + localStorage persistence, the JOSL event-stream panel, a11y & visual pass, deploy switchover.
