...
- [ ] **JSON Patch (RFC 6902) and JSON Merge Patch (RFC 7396)** — apply and structural diff, built on compiled pointers; a diff that emits JSON Patch doubles as a change feed for `@jarenjs/forms`.
- [ ] **Shared pointer-segment encode helper** — move forms' `escapePointerKey` (the write-side encode) into `@jarenjs/json/pointer` the next time the package opens, alongside the write operations above.
