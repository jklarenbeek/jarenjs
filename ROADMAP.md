...
- [ ] **Write operations** — `set`/`insert`/`remove` at a pointer, a normalized path, or every node a JSONPath query selects, with a copy-on-write mode. The pointer-addressed copy-on-write core now exists inside `src/patch.js`; publishing standalone setters and the JSONPath-selected variant remains.
