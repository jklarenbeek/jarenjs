...
- [QUIRKS.md](docs/QUIRKS.md) — the quirk hunt: the evidence-first audit for what is wrong but not loud — non-idempotent syncs, counts that lie, divergent twin helpers, comments that assert falsehoods, dead flags, wrong status codes. Sweep by area in parallel, reproduce every claim, fix with a regression test, and hand back three lists (fixed / not changed with reason / checked and dropped).
