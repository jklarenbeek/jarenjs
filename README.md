...
### @jarenjs/json — <!--fact:json.engines-->6<!--/fact--> compiled engines, <!--fact:json.suites-->5<!--/fact--> benchmarks

- **JSONPath** (`npm run benchmark:jsonpath`, `:profile`): **<!--fact:jsonpath.ctsPass-->all 703<!--/fact--> tests** of the official [JSONPath Compliance Test Suite](https://github.com/jsonpath-standard/jsonpath-compliance-test-suite) pass (normalized paths included; json-p3 scores <!--fact:jsonpath.ctsRival-->all 703<!--/fact-->). Performance vs [json-p3](https://www.npmjs.com/package/json-p3): **<!--fact:jsonpath.ctsRatio-->8.8<!--/fact-->x faster across the CTS queries**, as the geometric mean of the per-query ratios (<!--fact:jsonpath.ctsTimes-->109 ns vs 0.966 µs<!--/fact--> per query, the same summary over the same rows), and 4.9x (singular) to 81.8x (descendant `$..value`) on the synthetic 1000-item scenarios.
...
| [`@jarenjs/rules`](components/rules) | Schema-guided saved rule authoring, immutable previews and stable selection through injected services | [README](components/rules/README.md) · [ARCHITECTURE](components/rules/ARCHITECTURE.md) |
...
Measured on the committed 123-route table (`npm run benchmark:contract`): the matcher resolves the probe mix at <!--fact:contract.match.vs-fmw-->172 ns per lookup vs find-my-way's 185 ns<!--/fact--> — hono's TrieRouter is <!--fact:contract.match.vs-hono-->1.9x<!--/fact--> behind and its RegExpRouter refuses this table outright. The whole dispatch pipeline is <!--fact:contract.dispatch.vs-fastify-->2.8–11.9x<!--/fact--> faster than Fastify driven through its own `inject` (a number that includes Fastify's mock-stream harness, and the suite says so). The honest loss is published beside the wins: the bare pieces Fastify composes — find-my-way + Ajv + fast-json-stringify, no harness, no response validation — are <!--fact:contract.dispatch.losses-->2.5–11.4x<!--/fact--> faster than the pipeline that routes, decodes, validates both directions and settles every hostile input into a coded response (the wide end of the band is the bare `{ok:true}` route, where there is no work to amortize the pipeline against); over a real loopback socket the two stacks are level. See [packages/contract](packages/contract/README.md).
