...
| [`@jarenjs/ai`](packages/ai) | Browser-side AI: one OpenAI-compatible client (OpenRouter/Ollama/LM Studio, bring-your-own-key), an SSE decoder, a tool registry whose inputs Jaren validates before every call, a bounded agent loop, WebMCP registration — and a durable ledger: a persistent objective, evidenced memories and archived conversation rounds, so a compacted session can fetch back what it dropped and a run outlives the tab | [README](packages/ai/README.md) |
